...
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
...
// If a bottom or top wall shall move in negative x-direction, a negative
...
const int t_disk = 400; // disk write time step (data will be written to the disk every t_disk step)
const int t_info = 2000; // info time step (screen message will be printed every t_info step)
...
std::vector<std::vector<std::vector<double>>> pop;  // LBM populations (new)
std::vector<std::vector<std::vector<double>>> pop_old;  // LBM populations (old)
std::vector<std::vector<double>> density; // fluid density
std::vector<std::vector<double>> velocity_x; // fluid velocity (x-component)
std::vector<std::vector<double>> velocity_y; // fluid velocity (y-component)
...
struct particle_struct {
  ...
  particle_struct() {
    ...
    for(int n = 0; n < num_nodes; ++n) {
        ...
        node[n].x_ref = center.x + radius * sin(2. * M_PI * (double) n /
            num_nodes);
        node[n].y = center.y + radius * cos(2. * M_PI * (double) n / num_nodes);
        node[n].y_ref = center.y + radius * cos(2. * M_PI * (double) n /
            num_nodes);
      #endif
        ...
        if(node[n].y > 0) {
          node[n].y = center.y + sqrt(1 - SQ((center.x - node[n].x) / radius)) *
              (0.207 + 2.00 * SQ((center.x - node[n].x) / radius) - 1.12 *
              SQ(SQ((center.x - node[n].x) / radius))) * radius / 2;
          node[n].y_ref = center.y + sqrt(1 - SQ((center.x - node[n].x) /
              radius)) * (0.207 + 2.00 * SQ((center.x - node[n].x) / radius) -
              1.12 * SQ(SQ((center.x - node[n].x) / radius))) * radius / 2;
        }
        else {
          node[n].y = center.y - sqrt(1 - SQ((center.x - node[n].x) / radius)) *
              (0.207 + 2.00 * SQ((center.x - node[n].x) / radius) - 1.12 *
...
=======
  density.assign(Nx, std::vector<double>(Ny, 1.0));
  velocity_x.assign(Nx, std::vector<double>(Ny, 0.0));
  ...
  pop.assign(9, std::vector<std::vector<double>>(Nx, std::vector<double>(Ny,
      0.0)));
  pop_old.assign(9, std::vector<std::vector<double>>(Nx, std::vector<double>(Ny,
      0.0)));
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
...
void equilibrium(double den, double vel_x, double vel_y) {
  ...
  pop_eq[1] = weight[1] * den * (1 + 3 * vel_x + 4.5 *
      SQ(vel_x) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[2] = weight[2] * den * (1 + 3 * -vel_x + 4.5 *
      SQ(-vel_x) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[3] = weight[3] * den * (1 + 3 * vel_y + 4.5 *
      SQ(vel_y) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[4] = weight[4] * den * (1 + 3 * -vel_y + 4.5 *
      SQ(-vel_y) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[5] = weight[5] * den * (1 + 3 * (vel_x + vel_y) + 4.5 *
      SQ(vel_x + vel_y) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[6] = weight[6] * den * (1 + 3 * (-vel_x - vel_y) + 4.5 *
      SQ(-vel_x - vel_y) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[7] = weight[7] * den * (1 + 3 * (vel_x - vel_y) + 4.5 *
      SQ(vel_x - vel_y) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
  pop_eq[8] = weight[8] * den * (1 + 3 * (-vel_x + vel_y) + 4.5 *
      SQ(-vel_x + vel_y) - 1.5 * (SQ(vel_x) + SQ(vel_y)));
...
void LBM(int time) {
  ...
  /// Swap populations
  // The present code used old and new populations which are swapped at the
  ...
  // - combined collision and propagation (faster than first collision and then
  //   propagation)
  ...
  for(int X = 0; X < Nx; ++X) {
    for(int Y = 1; Y < Ny - 1; ++Y) {
      ...
      equilibrium(density[X][Y], (velocity_x[X][Y] + (force_x[X][Y] + gravity) *
          tau / density[X][Y]), (velocity_y[X][Y] + (force_y[X][Y]) * tau /
          density[X][Y]));
...
=======
      ...
      // propagation) including external forcing.
      // Periodicity of the lattice in x-direction is taken into account by the
      // %-operator.

      pop[0][X][Y] = pop_old[0][X][Y] * (1 - omega) + pop_eq[0] * omega;
      pop[1][(X + 1) % Nx][Y] = pop_old[1][X][Y] * (1 - omega) +
          pop_eq[1] * omega;
      pop[2][(X - 1 + Nx) % Nx][Y] = pop_old[2][X][Y] * (1 - omega) +
          pop_eq[2] * omega;
      pop[3][X][Y + 1] = pop_old[3][X][Y] * (1 - omega) + pop_eq[3] * omega;
      pop[4][X][Y - 1] = pop_old[4][X][Y] * (1 - omega) + pop_eq[4] * omega;
      pop[5][(X + 1) % Nx][Y + 1] = pop_old[5][X][Y] * (1 - omega) +
          pop_eq[5] * omega;
      pop[6][(X - 1 + Nx) % Nx][Y - 1] = pop_old[6][X][Y] * (1 - omega) +
          pop_eq[6] * omega;
      pop[7][(X + 1) % Nx][Y - 1] = pop_old[7][X][Y] * (1 - omega) +
          pop_eq[7] * omega;
      pop[8][(X - 1 + Nx) % Nx][Y + 1] = pop_old[8][X][Y] * (1 - omega) +
          pop_eq[8] * omega;
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
    }
  }
  ...
  // populations have to be bounced back.
  // Ladd's momentum correction term is included for moving walls (wall velocity
  // parallel to x-axis).
  // Periodicity of the lattice in x-direction is taken into account via the
  // %-operator.
  ...
  for(int X = 0; X < Nx; ++X) {
    ...
    pop[3][X][1] = pop[4][X][0];
    pop[5][X][1] = pop[6][(X - 1 + Nx) % Nx][0] + 6 * weight[6] *
        density[X][1] * wall_vel_bottom;
    pop[8][X][1] = pop[7][(X + 1) % Nx][0] - 6 * weight[7] *
        density[X][1] * wall_vel_bottom;
...
=======
    pop[4][X][Ny - 2] = pop[3][X][Ny - 1];
    pop[6][X][Ny - 2] = pop[5][(X + 1) % Nx][Ny - 1] - 6 * weight[5] *
...
=======
      density[X][Y] = pop[0][X][Y] + pop[1][X][Y] + pop[2][X][Y] +
          pop[3][X][Y] + pop[4][X][Y] + pop[5][X][Y] + pop[6][X][Y] +
          pop[7][X][Y] + pop[8][X][Y];
      velocity_x[X][Y] = (pop[1][X][Y] - pop[2][X][Y] + pop[5][X][Y] -
          pop[6][X][Y] + pop[7][X][Y] - pop[8][X][Y]) / density[X][Y];
      velocity_y[X][Y] = (pop[3][X][Y] - pop[4][X][Y] + pop[5][X][Y] -
          pop[6][X][Y] - pop[7][X][Y] + pop[8][X][Y]) / density[X][Y];
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
    }
  }
...
// Depending on the simulation type (rigid/deformable cylinder or red blood
// cell),
...
=======
  // The strain forces are proportional to the relative displacement of a node
  // with respect to its two neighbors.
  // This force is invariant under displacements and rotations,
  // i.e., it is only sensitive to deformations.
  // Newton's law is fulfilled: sum of forces is zero.
  // In order to make the force distribution smoother, each node is assigned an
  // equivalent area.
  // This area is the circumference of the cylinder divided by the number of
  // surface nodes.
  // WARNING: This is a simple strain model and not necessarily sufficient for
  // accurate simulations.

  #if defined DEFORMABLE_CYLINDER || defined DEFORMABLE_RBC
    // area belonging to a node
    const double area = 2 * M_PI * particle.radius / particle.num_nodes;
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
  ...
  for(int n = 0; n < particle.num_nodes; ++n) {
    ...
    for(int X = x_int; X <= x_int + 1; ++X) {
      for(int Y = y_int; Y <= y_int + 1; ++Y) {
        ...
        // Compute interpolation weights for x- and y-direction based on the
        // distance.
...
=======
        force_x[(X + Nx) % Nx][Y] += (particle.node[n].force_x * weight_x *
            weight_y);
        force_y[(X + Nx) % Nx][Y] += (particle.node[n].force_y * weight_x *
            weight_y);
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
      }
    }
...
=======
    // Identify the lowest fluid lattice node in interpolation range (see
    // spreading).
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
...
=======
        particle.node[n].vel_x += ((velocity_x[(X + Nx) % Nx][Y] + 0.5 *
            (force_x[(X + Nx) % Nx][Y] + gravity) / density[(X + Nx) % Nx][Y]) *
            weight_x * weight_y);
        particle.node[n].vel_y += ((velocity_y[(X + Nx) % Nx][Y] + 0.5 *
            (force_y[(X + Nx) % Nx][Y]) / density[(X + Nx) % Nx][Y]) *
            weight_x * weight_y);
>>>>>>> ba6d730e42c7359c93c8b7ade3efed2d6c62488a
      }
    }
...
// If the particle center leaves the system domain (x < 0 or x >= Nx), it
// reenters from the other side.
...
void update_particle_position(particle_struct particle) {
  ...
  particle.center.x = 0;
  particle.center.y = 0;
//  particle.center.x_ref = 0;
//  particle.center.y_ref = 0;
...
void write_fluid_vtk(int time) {
  ...
  for(int Y = 1; Y < Ny - 1; ++Y) {
    for(int X = 0; X < Nx; ++X) {
      output_file << velocity_x[X][Y] + 0.5 * (force_x[X][Y] + gravity) /
          density[X][Y] << " " << velocity_y[X][Y] + 0.5 * (force_y[X][Y]) /
          density[X][Y] << " 0\n";
    }
  }
